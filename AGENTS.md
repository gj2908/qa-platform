...
app's own `AGENTS.md` first — this file is just the map.

At the end of a work session that introduced a new table, cross-cutting
pattern, or gotcha, run the same maintenance skill used to keep
`CLAUDE.md` in sync (`.agents/skills/update-claude-md/SKILL.md`) so
these three files don't drift out of date with the codebase — mirror
any update into the corresponding `CLAUDE.md` file too, since the two
sets describe the same repo and should never disagree.

## Shape of the repo

Two independent Next.js (Pages Router) apps, one shared Supabase project:

- `main/` — the product itself. See `main/AGENTS.md`.
