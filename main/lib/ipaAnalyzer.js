...
export function analyzeIpa(buffer) {
  ...
  try {
    ...
    // Development + Ad Hoc both install OTA on devices whose UDID is in the
    // profile; Enterprise installs on any device. Only an App Store-style
    // profile (no devices, not in-house) cannot install OTA at all.
    let installType = "unknown";
    if (type === "Enterprise") installType = "any-device";
    else if (type === "Development" || type === "Ad Hoc") installType = "registered-devices";
    ...
    result.otaReady = installType !== "unknown";
  } catch (e) {
