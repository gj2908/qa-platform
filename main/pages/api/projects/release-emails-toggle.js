...
export default async function handler(req, res) {
  ...
  const { projectId, releaseEmailsEnabled } = req.body || {};
  if (!projectId) {
  ...
  const { error } = await authSupabase
    .from("projects")
    .update({ release_emails_enabled: !!releaseEmailsEnabled })
    .eq("id", projectId);
  ...
  await logActivity(createServiceClient(), {
    ...
    detail: `release_emails_enabled set to ${!!releaseEmailsEnabled}`,
